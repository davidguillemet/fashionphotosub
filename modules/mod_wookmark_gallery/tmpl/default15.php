	...
	if($image_fetch)
	{
		...
		$dir = opendir($filePath);
			...
			for($c=0;$c <count($string1) ; $c++)
			{
				?>
				<li>
						<a class="fancybox" data-fancybox-group="gallery" href="<?php echo $base_url.$filePath.$string1[$c]?>">
		...
		$link[$i]=trim($link[$i],chr(13));
			?>
				<li>
					<?php if(!$link_option) 
					{?>
					<a  class="fancybox" data-fancybox-group="gallery" href="<?php echo $base_url.$img_path[$i]?>" <?php  if($caption_option==1 || $caption_option==3) {?>title="<?php echo $caption[$i];?>"<?php }?> >
