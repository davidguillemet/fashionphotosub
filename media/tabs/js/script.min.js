...
$link.click(function(e){$this.openParents(tab_id,$anchor);e.stopPropagation();});},initHashHandling:function(el){$('a.nn_tabs-toggle').on('shown shown.bs.tab',function(e){var id=$(this).attr('data-id');var $el=$('#'+id);$el.attr('id','');window.location.hash=id;$el.attr('id',id);e.stopPropagation();});},initClickMode:function(el){$('body').on('click.tab.data-api','a.nn_tabs-toggle',function(e){e.preventDefault();nnTabs.show($(this).attr('data-id'),$(this).hasClass('nn_tabs-doscroll'));e.stopPropagation();});},initHoverMode:function(el){$('body').on('hover.tab.data-api','a.nn_tabs-toggle',function(e){e.preventDefault();nnTabs.show($(this).attr('data-id'));});},initCookieHandling:function(el){var $this=this;$('a.nn_tabs-toggle').on('show show.bs.tab',function(e){var id=$(this).attr('data-id');var $el=$this.getElement(id);var set=0;$el.closest('ul:not(.dropdown-menu)').each(function($i,el){set=el.id;});var obj={};var cookies=$.cookie(nn_tabs_cookie_name);if(cookies){cookies=cookies.split('___');for(var i=0;i<cookies.length;i++){var keyval=cookies[i].split('=');if(keyval.length>1&&keyval[0]!=set){var key=keyval.shift();if(key.substr(0,11)=='set-nn_tabs'){obj[key]=keyval.join('=');}}}}
