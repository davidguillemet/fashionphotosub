		...
		if (typeof( window['nn_sliders_use_hash'] ) != "undefined") {
			nnSliders = {
				init: function() {
					...
					this.insertCSSRule(['.nn_sliders .fade', '.nn_sliders .collapse'], 'transition-duration: 1ms');
				...
				scroll: function(id) {
					...
					switch (nn_sliders_scroll) {
						case 2:
							// Scroll to slider group
				...
				initClickMode: function(el) {
					$('body').on('click.collapse.data-api', 'a.nn_sliders-toggle', function(e) {
						e.preventDefault();
						nnSliders.show($(this).attr('data-id'), $(this).hasClass('nn_sliders-item-scroll'));
					});
				},
