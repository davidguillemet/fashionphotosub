			...
			<form action="" method="post" name="adminForm" id="adminForm">
				...
				<div id="filter-bar" class="btn-toolbar">
					<div class="filter-search btn-group pull-left">
						...
						<input type="text" name="filter_search" id="filter_search" placeholder="<?php echo JText::_('JSEARCH_FILTER'); ?>" value="<?php echo $lists['filter_search']; ?>" title="<?php echo JText::_('JSEARCH_FILTER'); ?>" />
					</div>
					...
					<tbody>
						...
						for ($i = 0, $n = count($rows); $i < $n; $i++)
						{
							...
							<tr class="<?php echo "row$k"; ?>">
								...
								<td>
									<?php echo '<label class="hasTip" title="{' . htmlspecialchars($tag) . ' ' . htmlspecialchars($item->alias) . '}"><a href="javascript://" onclick="snippets_jInsertEditorText( \'' . addslashes(htmlspecialchars($item->alias)) . '\' )">' . htmlspecialchars($item->alias) . '</a></label>'; ?>
								</td>
								<td>
