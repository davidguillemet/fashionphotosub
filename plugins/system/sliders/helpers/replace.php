...
class plgSystemSlidersHelperReplace
{
	...
	private function removeByAccess(&$string)
	{
		$regex = '#'
			...
			. '.*?'
			. '(?=\{(?:' . $this->params->tag_open . '|/' . $this->params->tag_close . '))'
			. '#s';
		...
		$user_levels = $this->getAccessLevels();
		...
		foreach ($matches as $match)
		{
			...
			switch (true)
			{
				case (isset($tag->usergroup)) :
					...
					$intersect = array_intersect($levels, $user_groups);
					break;

				case (isset($tag->access)) :
				default:
					$levels = explode(',', str_replace(' ', '', strtolower($tag->access)));
					$intersect = array_intersect($levels, $user_levels);
					break;
			}
	...
	private function getSliderTitle($item, $items)
	{
		...
		$class .= $item->scroll ? ' nn_sliders-item-scroll' : '';
	...
	{
		$onclick = 'nnSliders.show(this.rel, ' . (int) $this->params->linkscroll . ', 1);return false;';
