...
class plgSystemTabsHelper
{
	public function __construct(&$params)
	{
		...
		else
		{
			...
			$this->mainclass[] = $this->params->positioning;
			if ($this->params->positioning == 'top' || $this->params->positioning == 'bottom')
			{
	...
	function replaceTags(&$str, $area = 'article')
	{
				...
				foreach ($items as $i => &$item)
				{
					...
					if (preg_match($s, $str, $match))
					{
						...
						if ($i == $first)
						{
							...
							if (in_array('top', $item->classes))
							{
								$classes = array_diff($classes, array('top', 'bottom', 'left', 'right'));
								$item->classes = array_diff($item->classes, array('top', 'bottom', 'left', 'right'));
								$classes[] = 'top';
							}
							else if (in_array('bottom', $item->classes))
							{
								$classes = array_diff($classes, array('top', 'bottom', 'left', 'right'));
								$item->classes = array_diff($item->classes, array('top', 'bottom', 'left', 'right'));
								$classes[] = 'bottom';
							}
							else if (in_array('left', $item->classes))
							{
								$classes = array_diff($classes, array('top', 'bottom', 'left', 'right'));
								$item->classes = array_diff($item->classes, array('top', 'bottom', 'left', 'right'));
								$classes[] = 'left';
							}
							else if (in_array('right', $item->classes))
							{
								$classes = array_diff($classes, array('top', 'bottom', 'left', 'right'));
								$item->classes = array_diff($item->classes, array('top', 'bottom', 'left', 'right'));
								$classes[] = 'right';
		...
		if (preg_match_all($this->params->regex_end, $str, $matches, PREG_SET_ORDER) > 0)
		{
			...
			{
				$script .= 'nnTabs.scroll(\'' . $urlscroll . '\');';
			}
			if ($script)
			{
		...
		if (preg_match_all($this->params->regex_link, $str, $matches, PREG_SET_ORDER) > 0)
		{
			foreach ($matches as $match)
			{
				...
				if ($linkitem)
				{
					...
					$onclick = 'nnTabs.show(this.rel, ' . (int) $this->params->linkscroll . ');return false;';
					$html = '<a href="' . $href . '"'
	...
	function getNav(&$items)
	{
		...
		{
			// Nav for mobile view
			$html[] = '<ul class="nav nav-pills nav-stacked nav-tabs-sm" id="set-nn_tabs-sm-' . $items['0']->set . '">';
			foreach ($items as $item)
			{
				$html[] = '<li class="' . trim('nn_tabs-tab-sm ' . trim($item->class)) . '">';
				if ($item->haslink)
				{
		...
		foreach ($items as $item)
		{
			...
			else
			{
				$html[] = '<a href="#' . $item->id . '" data-toggle="tab"'
					. ($item->scroll ? ' onclick="nnTabs.scroll(\'' . $item->id . '\');"' : '')
					. '>'
