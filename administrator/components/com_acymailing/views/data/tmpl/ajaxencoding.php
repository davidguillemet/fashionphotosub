...
?>
	<table class="adminlist" cellspacing="10" cellpadding="10" align="center" id="importdata">
		<?php
			if($noHeader){
				$secondLine = $columnNames;
			}else{
				$secondLine = explode($this->separator, $this->lines[1]);
				foreach($secondLine as &$oneValue){
			...
			foreach($columnNames as $key => &$oneColumn){
				...
				if(empty($default) && $default !== 0){
					...
					if(!$default && !empty($secondLine)){
						if(strpos($secondLine[$key], '@')) $default = 'email';
