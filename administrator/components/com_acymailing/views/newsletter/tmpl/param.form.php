	...
	<?php echo $this->tabs->startPane( 'mail_tab');
		...
		<?php if(empty($this->lists)){
				...
				if(count($this->lists) > 10){
					...
					<?php
					echo '<div style="margin-bottom:10px;"><input onkeyup="acymailing_searchAList();" type="text" style="width:100%; margin-bottom:5px;" placeholder="'.JText::_('ACY_SEARCH').'" id="acymailing_searchList"></div>';
				}
		?>
						...
						<script language="javascript" type="text/javascript">
							...
							function updateStatus(selection){
								...
								for(var i=0; i < selectedLists['all'].length; i++)
								{
									...
									<?php
									if(ACYMAILING_J30){
										echo 'jQuery("input[id="+selectedLists["all"][i]+"listmail0]").click();';
									}
									?>
								...
								for(var i=0; i < selectedLists[selection].length; i++)
								{
									...
									<?php
									if(ACYMAILING_J30){
										echo 'jQuery("input[id="+selectedLists[selection][i]+"listmail1]").click();';
									}
									?>
